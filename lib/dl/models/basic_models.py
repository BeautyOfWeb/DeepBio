...
def squared_dist(x, y):
  ...
  Args:
    ...
    y: tensor of size (M, D)
    
  Returns:
    d: tensor of size (N, M)
    
  Examples:
    x = torch.randn(3, 7)
    y = torch.randn(5, 7)
    d = dist(x, y)
    torch.norm(d - torch.norm(x.unsqueeze(1)-y, dim=-1)**2)
  """
  return torch.sum(x*x, dim=-1, keepdim=True) + torch.sum(y*y, dim=-1) - 2 * torch.mm(x, y.t())
...
class PrototypicalNet(nn.Module):
  """Modified version of "Prototypical Networks for Few-shot Learning"
  During training, treat it as DenseLinear model; 
    then save the average output (must be a large batch) of the last layer as class centroids;
  During evaluation, use stored class centroids as prototypes, and calculate pairwise distance 
    between new samples and these prototypes; 
    this NEGATIVE pairwise distance matrix will be the class score for cross entropy loss
  
  Args:
    Most arguments passed to DenseLinear model
    num_cls is absorbed in DenseLinear hidden_dim, but here is separated
    requires_grad_prototype: if True, learn self.prototype, otherwise use modified PrototypicalNetwork Algorithm 1
    
  Examples:
    in_dim = 20
    hidden_dim = [30]
    num_cls = 3
    model = PrototypicalNet(in_dim, hidden_dim, num_cls, nonlinearity=nn.ReLU(), 
                   last_nonlinearity=False, bias=True, dense=False, residual=False,
                  residual_layers='all-but-last')
    N = 11
    x_train = torch.randn(N, in_dim)
    y_train = torch.randint(num_cls, (N,)).long()
    x_val = torch.randn(N, in_dim)
    y_val = torch.randint(num_cls, (N,)).long()
    x_test = torch.randn(N, in_dim)
    y_test = torch.randint(num_cls, (N,)).long()

    train_single_loss(model, x_train=x_train, y_train=y_train, x_val=x_val, y_val=y_val, 
                      x_test=x_test, y_test=y_test, 
        loss_fn=nn.CrossEntropyLoss(), lr=1e-2, weight_decay=1e-4, amsgrad=True, batch_size=None, 
                      num_epochs=500, 
        reduce_every=200, eval_every=1, print_every=100, verbose=False, 
        loss_train_his=[], loss_val_his=[], loss_test_his=[], 
        acc_train_his=[], acc_val_his=[], acc_test_his=[], return_best_val=True, 
        forward_kwargs_train={'y':y_train, 'train':True}, 
                      forward_kwargs_val={}, forward_kwargs_test={})
                      
  """
  def __init__(self, in_dim, hidden_dim, num_cls, nonlinearity=nn.ReLU(), 
               last_nonlinearity=False, bias=True, dense=False, residual=False,
              residual_layers='all-but-last', requires_grad_prototype=False):
    super(PrototypicalNet, self).__init__()
    self.num_cls = num_cls
    self.requires_grad_prototype = requires_grad_prototype
    self.transform = DenseLinear(in_dim, hidden_dim, nonlinearity=nonlinearity, 
                last_nonlinearity=last_nonlinearity, bias=bias, dense=dense, 
                residual=residual, residual_layers=residual_layers, 
                forward_input=False, return_all=False, return_layers=None, return_list=False)
    self.prototype = nn.Parameter(torch.randn(self.num_cls, hidden_dim[-1]), 
        requires_grad=requires_grad_prototype)
    
  def forward(self, x, y=None, train=False):
    """During training, set train to be True, and provide a sufficiently large sample that contain
        all classes; assume y has class label from 0 to num_cls-1; 
        for each class there are at least a few instances
      During evaluation, set train=False, y will not be used
    """
    score = self.transform(x)
    if train and not self.requires_grad_prototype:
      centroids = []
      for c in range(self.num_cls):
        centroids.append(score[y==c].mean(dim=0))
      self.prototype.data = torch.stack(centroids, dim=0)
    score = - squared_dist(score, self.prototype)
    return score